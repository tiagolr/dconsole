...
/** Aligns console to top */
	...
	private static var commandNames:Array<String> = { ["clear", "monitor", "help", "commands", "vars", "fcns", "set"];};
...
// INPUT PARSE ----------------------------------------------------
	...
	private function parseInput(input:String) {
		...
		switch (argsLC[0]) {
			...
			default :log(GCCommands.callFunction(args));
		}
	}
...
// COMPLETION --------------------------------------------------------
	...
	private function completeInputLine() {
		...
		var argsLC:Array<String> = temp.toLowerCase().split(' ');
		trace(commandNames);
		var functions = GCCommands.getFunctionNames().concat(commandNames);
